...
- [x] 03-05-PLAN.md — The draw gesture: pointer handlers, live preview, edge clamping, silent per-type rejection, and the insert → get id commit path (wave 3, human-verify checkpoint)
...
- [x] 04-01-PLAN.md — The write path: `FigureStore.UpdateAsync`/`DeleteAsync` via `ExecuteUpdateAsync`/`ExecuteDeleteAsync`, owner-filtered and affected-row-count returning, with the IDOR and zero-row proofs (wave 1)
- [x] 04-02-PLAN.md — The presentation surfaces: `FigureShape`'s `Selected`/`Selectable`/`OnPointerDown` and the red outline; `Toolbar`'s `OnDelete` (wave 1)
...
- [x] 04-04-PLAN.md — Human verification of the five success criteria on a real screen (wave 3, blocking checkpoint)
...
- [x] 05-03-PLAN.md — `Home.razor`'s sync wiring: subscribe/dispose, echo filter, D-54's blanket mid-drag discard, D-40's update-only apply, the 50ms glide with its guaranteed trailing edge, and the zero-row delete broadcast (wave 3)
