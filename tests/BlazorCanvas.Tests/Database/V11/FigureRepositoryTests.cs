...
public class FigureRepositoryTests
{
    ...
    private async Task<FigureSnapshot> ReadSnapshotAsync(Guid figureId)
    {
        const string sql = """
            ...
            FROM public.figures WHERE id = @id
            """;
        await using var connection = await _fixture.OpenV11ConnectionAsync();
    ...
    {
        const string sql = "INSERT INTO public.figure_types (name) VALUES (@name) ON CONFLICT (name) DO NOTHING";
        await using var connection = await _fixture.OpenV11ConnectionAsync();
        await using var command = new NpgsqlCommand(sql, connection);
    ...
    {
        const string deleteFigureSql = "DELETE FROM public.figures WHERE id = @id";
        const string deleteTypeSql = "DELETE FROM public.figure_types WHERE name = @name";
        await using var connection = await _fixture.OpenV11ConnectionAsync();
...
internal static class V11JsonAssertions
{
        ...
        string expectedJson)
    {
        ...
        var sql = $"SELECT {column} = @expected::jsonb FROM public.figures WHERE id = @id";
        await using var command = new NpgsqlCommand(sql, connection);
