...
public class CanvasRepositoryTests
{
    ...
    {
        await using var connection = await _fixture.OpenV11ConnectionAsync();
        await using var command = new NpgsqlCommand("SELECT count(*) FROM v11.canvases WHERE owner_id = @owner_id", connection);
        command.Parameters.AddWithValue("owner_id", ownerId);
    ...
    {
        await using var connection = await _fixture.OpenV11ConnectionAsync();
        await using var command = new NpgsqlCommand("SELECT count(*) FROM v11.figures WHERE canvas_id = @canvas_id", connection);
