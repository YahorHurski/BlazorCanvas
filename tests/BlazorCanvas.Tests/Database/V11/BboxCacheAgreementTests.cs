...
public class BboxCacheAgreementTests
{
    ...
    public async Task AgreementGuardDetectsDeliberatelyCorruptedCache(string column)
    {
        ...
        await using var command = new NpgsqlCommand($"UPDATE v11.figures SET {column} = {column} + 1 WHERE id = @id", connection, transaction);
    ...
    public async Task NegativeBboxExtentIsRefusedByTheDatabase()
    {
        ...
        var exception = await Assert.ThrowsAsync<PostgresException>(() =>
            ExecuteRawInsertAsync(canvasId, "INSERT INTO v11.figures (id, canvas_id, type, x, y, rotation, geometry, style, z, bbox_x, bbox_y, bbox_w, bbox_h) VALUES (@id, @canvas, 'rectangle', 0, 0, 0, '{\"w\":20,\"h\":10}'::jsonb, '{}'::jsonb, 1, 0, 0, -1, 10)"));
    ...
    public async Task EveryBboxColumnIsRequired()
    {
        var canvasId = await CreateCanvasAsync();
        var exception = await Assert.ThrowsAsync<PostgresException>(() =>
            ExecuteRawInsertAsync(canvasId, "INSERT INTO v11.figures (id, canvas_id, type, x, y, rotation, geometry, style, z, bbox_x, bbox_y, bbox_w) VALUES (@id, @canvas, 'rectangle', 0, 0, 0, '{\"w\":20,\"h\":10}'::jsonb, '{}'::jsonb, 1, 0, 0, 20)"));
