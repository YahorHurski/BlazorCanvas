...
public class ClampTests
{
    ...
    public void FlushRightEdge_XClippedToZero_YPassesThroughAtFullDelta()
    {
        ...
        var result = Movement.ClampMove(new Box(1372, 300, 1472, 400), dx: 50, dy: -30);

        Assert.Equal(new Box(1372, 270, 1472, 370), result);
    }

    [Fact]
    public void FlushBottomEdge_YClippedToZero_XPassesThroughAtFullDelta()
    {
        var result = Movement.ClampMove(new Box(300, 728, 400, 828), dx: 40, dy: 60);

        Assert.Equal(new Box(340, 728, 440, 828), result);
    }

    [Fact]
    public void AlreadyTouchingBothMaxima_DoesNotMoveAtAll()
    {
        // X2 == 1472 and Y2 == 828 are legal, reachable positions, not overflow (D-19).
        var result = Movement.ClampMove(new Box(1392, 708, 1472, 828), dx: 1, dy: 1);

        Assert.Equal(new Box(1392, 708, 1472, 828), result);
    }

    [Fact]
    public void LargeDownwardDelta_LandsExactlyAtY2Equals828()
    {
        var result = Movement.ClampMove(new Box(1372, 708, 1472, 808), dx: 0, dy: 100);

        Assert.Equal(new Box(1372, 728, 1472, 828), result);
        Assert.Equal(828, result.Y2);
    }
    ...
    public void LineWithY1GreaterThanY2_ClampsAgainstTheMinMaxBoundingBox()
    {
        ...
        // mis-clamps.
        var result = Movement.ClampMove(new Box(0, 808, 100, 100), dx: 0, dy: 100);

        Assert.Equal(new Box(0, 828, 100, 120), result);
    }

    public static IEnumerable<object[]> ShapeInvarianceCases()
    {
        yield return new object[] { new Box(1372, 300, 1472, 400), 50, -30 };
        yield return new object[] { new Box(300, 728, 400, 828), 40, 60 };
        yield return new object[] { new Box(1392, 708, 1472, 828), 1, 1 };
        yield return new object[] { new Box(1372, 708, 1472, 808), 0, 100 };
        yield return new object[] { new Box(0, 0, 50, 50), -1, -1 };
        yield return new object[] { new Box(10, 10, 60, 60), -100, 0 };
        yield return new object[] { new Box(0, 808, 100, 100), 0, 100 };
        // Deltas far outside the canvas in every direction.
        yield return new object[] { new Box(500, 500, 600, 600), 100_000, 100_000 };
        yield return new object[] { new Box(500, 500, 600, 600), -100_000, -100_000 };
        yield return new object[] { new Box(0, 0, 1472, 828), 0, 0 };
    }
    ...
    {
        // CR-02: width 2000 exceeds the canvas width of 1472, so lo (-bx1) > hi (W - bx2).
    ...
    {
        // CR-02 y-axis mirror: height 900 exceeds the canvas height of 828.
    ...
    {
        Assert.Equal(0, Movement.ClampDelta(0, 0, -828));
        Assert.Equal(0, Movement.ClampDelta(50, 0, -828));
        Assert.Equal(0, Movement.ClampDelta(-50, 0, -828));
    }
}
